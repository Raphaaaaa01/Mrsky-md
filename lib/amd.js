    ...
    <div class="main">
      <div class="box" id="box">
        <div id="text">
          ...
          <p>
            <h3 class="centered-text">MRSKY MD PAIRING CODE</h3>
            <br>
            <h6>Made By Mrsky tech💡💫.</h6>
            <h6>Enter Your Number with Country Code.</h6>
            <div class="input-container">
              <input placeholder="255698101xxx" type="number" id="number" placeholder="Enter your Phone Pumber with Cuntry Code" name="">
  ...
  <script>
    ...
    b.addEventListener("click", async (e) => {
      ...
      if (!c.value) {
        ...
        const Mrsky_Tech = c.value.replace(/[^0-9]/g, "");
        let bb = "";
        let bbc = "";
        const cc = Mrsky_Tech.split('');
        cc.map(a => {
          bbc += a;
          if (bbc.length == 3) {
            bb += " " + a;
        ...
        let { data } = await axios(`/code?number=${Mrsky_Tech}`);
...
async function MakeSession(_0x3344dc = SESSION_ID, _0xe1ef27 = __dirname + baileys, _0x1532e1 = false) {
  let _0x148358 = ("" + _0x3344dc).replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^wasi_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/MRSKY-MD;;;/gi, "").replace(/Astro;;;/gi, "").replace(/mrsky-cyber;;;/gi, "").trim();
